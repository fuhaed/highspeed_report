...
def execute(filters=None):
    ...
    if not filters.from_date or not filters.to_date:
        try:
            year_dates = frappe.db.get_value("Fiscal Year", 
                                            filters.fiscal_year, 
                                            ["year_start_date", "year_end_date"])
            
            # Only unpack if we got a valid result
            if year_dates and isinstance(year_dates, (list, tuple)) and len(year_dates) == 2:
                from_date, to_date = year_dates
                filters.from_date = from_date
                filters.to_date = to_date
            else:
                # Fallback to current fiscal year
                current_fiscal_year = frappe.get_all("Fiscal Year", 
                                                   filters={"disabled": 0},
                                                   order_by="year_start_date desc",
                                                   limit=1)
                if current_fiscal_year:
                    fiscal_year = frappe.get_doc("Fiscal Year", current_fiscal_year[0].name)
    ...
    if not filters.get("show_zero_values"):
        filtered_data = [row for row in filtered_data if (
            (row.get('account') and (
                flt(row.get('opening_debit')) != 0 or
                flt(row.get('opening_credit')) != 0 or
                flt(row.get('debit')) != 0 or
                flt(row.get('credit')) != 0 or
                flt(row.get('closing_debit')) != 0 or
                flt(row.get('closing_credit')) != 0
            )) or
            # Keep total row
...
def get_updated_columns(columns):
    ...
    column_map = {
        'account': {'label': _('Account'), 'fieldtype': 'Link', 'options': 'Account', 'width': 300},
        'opening_debit': {'label': _('Opening (Dr)'), 'fieldtype': 'Currency', 'width': 120},
        'opening_credit': {'label': _('Opening (Cr)'), 'fieldtype': 'Currency', 'width': 120},
        'debit': {'label': _('Debit'), 'fieldtype': 'Currency', 'width': 120},
        'credit': {'label': _('Credit'), 'fieldtype': 'Currency', 'width': 120},
        'closing_debit': {'label': _('Closing (Dr)'), 'fieldtype': 'Currency', 'width': 120},
        'closing_credit': {'label': _('Closing (Cr)'), 'fieldtype': 'Currency', 'width': 120}
    }
    
    updated_columns = []
    
    # You can reorder or modify the columns based on your requirements
    for col_key in ['account', 'opening_debit', 'opening_credit', 'debit', 'credit', 'closing_debit', 'closing_credit']:
        if col_key in column_map:
            # Find if the column already exists in the original columns
            existing_col = next((col for col in columns if col.get('fieldname') == col_key), None)
...
def filter_by_parent_account(data, parent_filter):
    ...
    if total_row:
        ...
        total_opening_debit = sum(flt(row.get('opening_debit', 0)) for row in filtered_data)
        total_opening_credit = sum(flt(row.get('opening_credit', 0)) for row in filtered_data)
        ...
        total_closing_debit = sum(flt(row.get('closing_debit', 0)) for row in filtered_data)
        total_closing_credit = sum(flt(row.get('closing_credit', 0)) for row in filtered_data)
...
def add_indentation(data):
    ...
    # First, let's extract all accounts that have parent accounts and exist in DB
    for row in data:
        account_name = row.get('account')
        if account_name and frappe.db.exists('Account', account_name):
            account_map[account_name] = {'row': row, 'level': 0}

frappe.query_reports["Parent Accounts Trial Balance"] = {
    ...
    "onload": function(report) {
        ...
        message.innerHTML = '<strong>' + __("Note") + ': </strong>' + 
            __("This report shows trial balance for accounts under the selected parent account. Select a parent account from the filter to view its trial balance.");
        ...
        var reportHeader = $(`
            <div class="print-format-information" style="display:none;">
                ...
                <div style="text-align: center; margin-bottom: 10px;">
                    <strong>${__("Company")}:</strong> ${frappe.query_report.get_filter_value('company')}
                </div>
                <div style="text-align: center; display: flex; justify-content: center; margin-bottom: 15px;">
                    <div style="margin-right: 30px;">
                        <strong>${__("From Date")}:</strong> 
                        ${frappe.format(frappe.query_report.get_filter_value('from_date'), {fieldtype: 'Date'})}
                    </div>
                    <div>
                        <strong>${__("To Date")}:</strong> 
                        ${frappe.format(frappe.query_report.get_filter_value('to_date'), {fieldtype: 'Date'})}
                    </div>
                </div>
                <div style="text-align: center; margin-bottom: 10px;">
                    <strong>${__("Parent Account")}:</strong> 
                    ${frappe.query_report.get_filter_value('parent_account_filter') || __("All Accounts")}
                </div>
            </div>
        `);

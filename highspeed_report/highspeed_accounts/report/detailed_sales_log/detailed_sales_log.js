            ...
            frappe.db.get_value('Company', filters.company, ['company_name', 'tax_id'], function(companyInfo) {
                ...
                data.forEach(function(row) {
                    if (!row.is_total_row) {
                        ...
                        var rowClass = credit_return_status ? 'style="background-color: #fff0f0;"' : '';
                        var returnInfo = return_against ? `<br><small>(مرتجع مقابل: ${return_against})</small>` : '';
                        var creditReturnBadge = credit_return_status ? `<br><span style="color: white; background-color: #dc3545; padding: 2px 5px; border-radius: 3px; font-size: 8px;">${credit_return_status}</span>` : '';
...
function downloadCSV(data, filename) {
    ...
    // Prepare headers
    var headers = ["التاريخ", "الوقت", "رقم المستند", "نوع المستند", "مرتجع مقابل", __("Customer"), "حالة الفاتورة", __("Tax"), __("Amount"), "نقطة البيع", "المستخدم", "المستودع", __("Cost Center"), __("Payment Mode"), "حالة مرتجع الآجل"];

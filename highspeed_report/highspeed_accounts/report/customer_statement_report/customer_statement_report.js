...
frappe.query_reports["Customer Statement Report"] = {
    ...
    "formatter": function(value, row, column, data, default_formatter) {
        if (data) {
                ...
                switch(value) {
                    ...
                    case "Payment Entry":
                        ...
                        if (data.payment_type === "استلام") {
                            paymentType = '<span style="color: green; font-weight: bold;">&#128176; سند استلام</span>';
                        } else if (data.payment_type === "دفع") {
                            paymentType = '<span style="color: red; font-weight: bold;">&#128184; سند دفع</span>';
                        } else {
                            paymentType = 'سند قبض';
                        }
                        return paymentType;
            ...
            if (column.fieldname === "balance") {
                ...
                return '<span style="color: ' + (value < 0 ? 'red' : 'blue') + ';">' + 
                       default_formatter(Math.abs(value), row, column, data) + ' ' + 
                       (value < 0 ? 'دائن' : 'مدين') + '</span>';
            }
    ...
    "onload": function(report) {
        ...
        var $customerSummarySection = $('<div class="customer-summary-section" style="margin-bottom: 20px;"></div>');
        ...
        var $customerInfoSection = $('<div class="customer-info" style="margin-top: 20px; margin-bottom: 20px;"></div>');
        ...
        $customerSummarySection.append($customerInfoSection).append($balanceCardsSection).append($transactionsSummary);
        ...
        function updateSelectedFilters() {
            ...
            if (filters.users && filters.users.length) {
                filtersHTML += '<div style="margin-bottom: 5px;"><strong>' + __("المستخدمين: ") + '</strong>';
            ...
            if (filters.cost_centers && filters.cost_centers.length) {
                filtersHTML += '<div style="margin-bottom: 5px;"><strong>' + __("مراكز التكلفة: ") + '</strong>';
            ...
            if (filters.warehouses && filters.warehouses.length) {
                filtersHTML += '<div style="margin-bottom: 5px;"><strong>' + __("المستودعات: ") + '</strong>';
        ...
        function updateCustomerInfo(filters) {
            ...
            frappe.db.get_value('Customer', filters.customer, 
                ['customer_name', 'customer_group', 'territory', 'tax_id', 'customer_type', 'payment_terms'], 
                function(r) {
                    if (r) {
                        frappe.call({
        ...
        report.page.add_inner_button(__("تصدير"), function() {
            if (report.data && report.data.length > 0) {
                ...
                downloadCSV(data, "customer_statement_" + filters.customer + "_" + filters.from_date + "_to_" + filters.to_date + ".csv");
            } else {
    ...
    return new Promise(function(resolve, reject) {
        try {
            frappe.db.get_value('Company', filters.company, 
                ...
                frappe.db.get_value('Customer', filters.customer, ['customer_name', 'tax_id'], function(customerInfo) {
                    ...
                    var filtersInfoHTML = '';
                    if (filters.users && filters.users.length) {
                        filtersInfoHTML += '<tr><td style="background-color: #f2f2f2; font-weight: bold; text-align: right;">' + __("المستخدمين") + ':</td><td colspan="3">' + filters.users.join(', ') + '</td></tr>';
                    }
                    if (filters.cost_centers && filters.cost_centers.length) {
                        filtersInfoHTML += '<tr><td style="background-color: #f2f2f2; font-weight: bold; text-align: right;">' + __("مراكز التكلفة") + ':</td><td colspan="3">' + filters.cost_centers.join(', ') + '</td></tr>';
                    }
                    if (filters.warehouses && filters.warehouses.length) {
                        filtersInfoHTML += '<tr><td style="background-color: #f2f2f2; font-weight: bold; text-align: right;">' + __("المستودعات") + ':</td><td colspan="3">' + filters.warehouses.join(', ') + '</td></tr>';
                    }
...
function downloadCSV(data, filename) {
    ...
    var headers = ["التاريخ", "رقم المستند", "نوع المستند", __("Particulars"), "حالة المستند", "نوع السند", "مدين", "دائن", __("Balance")];

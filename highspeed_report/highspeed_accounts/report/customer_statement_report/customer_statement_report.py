...
def get_customer_ledger_entries(filters):
    receivable_accounts = get_receivable_accounts(filters.get("company"), filters.get("customer"))
    ...
    for account in receivable_accounts:
        ...
        opening_values = [account, filters.get("customer"), filters.get("company"), opening_date]
    ...
    if opening_balance == 0:
        try:
            current_balance = frappe.db.get_value("Customer", filters.get("customer"), "outstanding_amount")
            if current_balance:
    ...
    for account in receivable_accounts:
        ...
        gl_values = [account, filters.get("customer"), filters.get("company"), 
    ...
    payment_entries = [entry.voucher_no for entry in gl_entries 
    ...
    for entry in gl_entries:
        ...
        if entry.voucher_type == "Payment Entry" and entry.voucher_no in payment_references:
            ...
            if references:
                invoices = [ref.reference_name for ref in references if ref.reference_doctype == "Sales Invoice"]
                if invoices:
        ...
        invoice_status = ""
        if entry.voucher_type == "Sales Invoice" and entry.voucher_no in invoice_statuses:
...
def get_invoice_statuses(invoice_numbers):
    ...
    invoices = frappe.get_all(
        ...
        fields=["name", "status", "is_return", "outstanding_amount", "grand_total", "is_pos"]
    )

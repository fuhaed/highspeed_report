...
def get_sales_data(filters):
    ...
    if filters.get("cost_center"):
        if isinstance(filters["cost_center"], str):
            # لو مستخدم value واحدة فقط كنص
            values["cost_center"] = (filters["cost_center"],)
        else:
            # لو MultiSelect بيرجع قائمة
            values["cost_center"] = tuple(filters["cost_center"])
    
        conditions.append("si.cost_center IN %(cost_center)s")
    
    # Filter by POS Profile if provided
    #if filters.get("pos_profile"):
       # conditions.append("si.pos_profile IN %(pos_profile)s")
        #values["pos_profile"] = tuple(filters["pos_profile"])
    
    if filters.get("pos_profile"):
        if isinstance(filters["pos_profile"], str):
            # لو مستخدم value واحدة فقط كنص
    ...
    if filters.get("owner"):
        if isinstance(filters["owner"], str):
            ...
            # لو MultiSelect بيرجع قائمة
            values["owner"] = tuple(filters["owner"])
    
        conditions.append("si.owner IN %(owner)s")
    ...
    if filters.get("warehouse"):
        if isinstance(filters["warehouse"], str):
            ...
            # لو MultiSelect بيرجع قائمة
            values["warehouse"] = tuple(filters["warehouse"])
    
        conditions.append("si_item.warehouse IN %(warehouse)s")
    ...
    if filters.get("show_credit_returns"):
        ...
        conditions.append("si.return_against IS NOT NULL")  # Must have a return_against value
    ...
    for row in sales_data:
        ...
        if row.is_return and row.return_against:
            ...
            original_is_pos = 0 if row.original_is_pos is None else int(row.original_is_pos)
            ...
            if is_credit_transaction:
                ...
                # إعداد الحالة المبدئية لقيمة المديونية
                
            
           # تأكد من تهيئة المتغيرات أولاً
                original_outstanding = 0.0
                credit_return_status = None
                is_credit_transaction = False  # احتياطي لو محتاجه في اللوج

                # التحقق إن الفاتورة دي إرجاع وليها فاتورة أصلية
                if row.get("return_against"):
                    # نحاول نقرأ قيمة المديونية
                    original_outstanding_amount = row.get("original_outstanding_amount")

                    if original_outstanding_amount is not None:
                        try:
                            original_outstanding = float(original_outstanding_amount)
                        except (ValueError, TypeError):
                            original_outstanding = 0.0

                    # لو المبلغ أكبر من صفر، معناها إرجاع لآجل غير مسدد
                    if original_outstanding > 0:
                        credit_return_status = _("فاتورة آجلة غير مسددة")

                # إضافة النتيجة إلى الصف
                row["credit_return_status"] = credit_return_status

                # تسجيل المعلومات للتصحيح
                frappe.logger().debug(
                    f"CREDIT_RETURN_CHECK: Return={row.get('voucher_no')}, Original={row.get('return_against')}, "
        ...
        voucher_type = _("Sales Invoice") if row.voucher_type == "Sales Invoice" else (row.voucher_type or "")
        ...
        data.append({
            ...
            "is_credit_return": 1 if credit_return_status else 0  # New field to make filtering easier
        })

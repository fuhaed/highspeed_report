...
frappe.query_reports["Payment Register Report"] = {
        ...
        if (data) {
            ...
            if (column.fieldname === "payment_type") {
                ...
                if (value && value.includes("سند قبض")) {
                    color = "green";
                } else if (value && value.includes(__("Payment Entry"))) {
                    color = "red";
            ...
            if (column.fieldname === "paid_amount") {
                ...
                else if (data.payment_type && data.payment_type.includes(__("Payment Entry"))) {
    ...
    "onload": function(report) {
        ...
        var $paymentSummaryHeader = $('<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 2px solid #f0f0f0; padding-bottom: 10px;"><h3 style="margin: 0; color: #333; font-weight: bold;">' + __("ملخص المقبوضات والمدفوعات") + '</h3><div class="report-date"></div></div>');
        ...
        // إنشاء بطاقات الملخص
        var $receiptCard = $('<div class="summary-card receipt" style="flex: 1;"><div class="summary-card-title">' + __("إجمالي المقبوضات") + '</div><div class="summary-amount receipt-amount"></div></div>');
        var $paymentCard = $('<div class="summary-card payment" style="flex: 1;"><div class="summary-card-title">' + __("إجمالي المدفوعات") + '</div><div class="summary-amount payment-amount"></div></div>');
        var $netCard = $('<div class="summary-card net" style="flex: 1;"><div class="summary-card-title">' + __("صافي الحركة") + '</div><div class="summary-amount net-amount"></div></div>');
        ...
        function updatePaymentSummary(data, filters) {
            ...
            data.forEach(function(row) {
                if (!row.is_receipt_total && !row.is_payment_total && !row.is_net_total) {
                    if (row.payment_type && row.payment_type.includes("سند قبض")) {
                        ...
                        receiptCount++;
                    } else if (row.payment_type && row.payment_type.includes(__("Payment Entry"))) {
            ...
            $(".net-amount").html(`<span style="color: ${netColor};">${format_currency(Math.abs(netAmount))}</span> <span style="font-size: 14px;">${netStatus}</span>`);
            ...
            data.forEach(function(row) {
                if (!row.is_receipt_total && !row.is_payment_total && !row.is_net_total) {
                    ...
                    if (row.mode_of_payment) {
                        ...
                        if (row.payment_type && row.payment_type.includes("سند قبض")) {
                            modeOfPaymentData[row.mode_of_payment].receipts += flt(row.paid_amount);
                        } else if (row.payment_type && row.payment_type.includes(__("Payment Entry"))) {
                            modeOfPaymentData[row.mode_of_payment].payments += flt(row.paid_amount);
                        }
                    }
            ...
            if (Object.keys(modeOfPaymentData).length > 0) {
                ...
                var $modeOfPaymentTable = $('<table class="table table-bordered" style="width: 100%; margin-bottom: 20px;"><thead><tr><th>' + __("Payment Mode") + '</th><th>' + __("المقبوضات") + '</th><th>' + __("Payments") + '</th><th>' + __("Net") + '</th></tr></thead><tbody></tbody></table>');
                ...
                Object.keys(modeOfPaymentData).forEach(function(mode) {
                    ...
                    $modeOfPaymentTable.find('tbody').append(`
                        <tr>
                            <td>${mode}</td>
                            <td style="color: green;">${format_currency(modeOfPaymentData[mode].receipts)}</td>
                            <td style="color: red;">${format_currency(modeOfPaymentData[mode].payments)}</td>
            ...
            if (Object.keys(partyTypeData).length > 0) {
                $analysisSection.append('<h4 style="margin-bottom: 15px; color: #333; font-weight: bold;">' + __("التحليل حسب نوع الطرف") + '</h4>');
                var $partyTypeTable = $('<table class="table table-bordered" style="width: 100%; margin-bottom: 20px;"><thead><tr><th>' + __("نوع الطرف") + '</th><th>' + __("المقبوضات") + '</th><th>' + __("Payments") + '</th><th>' + __("Net") + '</th></tr></thead><tbody></tbody></table>');
                ...
                Object.keys(partyTypeData).forEach(function(type) {
                    ...
                    $partyTypeTable.find('tbody').append(`
                        <tr>
                            <td>${type}</td>
                            <td style="color: green;">${format_currency(partyTypeData[type].receipts)}</td>
                            <td style="color: red;">${format_currency(partyTypeData[type].payments)}</td>
                            <td style="color: ${netColor};">${format_currency(Math.abs(net))} ${net >= 0 ? "فائض" : "عجز"}</td>
                        </tr>
                    `);
                });
        ...
        report.page.add_inner_button(__("تصدير"), function() {
            if (report.data && report.data.length > 0) {
                ...
                downloadCSV(data, "payment_register_" + filters.from_date + "_to_" + filters.to_date + ".csv");
            } else {
            ...
            frappe.db.get_value('Company', filters.company, ['company_name', 'tax_id'], function(companyInfo) {
                    ...
                    if (!row.is_receipt_total && !row.is_payment_total && !row.is_net_total) {
                        ...
                        paymentRows.push(`
                            <tr>
                                ...
                                <td style="${row.payment_type && row.payment_type.includes("سند قبض") ? 'color: green;' : row.payment_type && row.payment_type.includes(__("Payment Entry")) ? 'color: red;' : ''}">${format_currency(row.paid_amount)}</td>
                                <td>${row.account || ""}</td>
                ...
                paymentRows.push(`
                    ...
                    <tr style="font-weight: bold; background-color: #f2f2f2;">
                        ...
                        <td style="color: ${netAmount >= 0 ? 'green' : 'red'};">${format_currency(Math.abs(netAmount))} ${netAmount >= 0 ? 'فائض' : 'عجز'}</td>
                        <td colspan="5"></td>
                    </tr>
                `);
...
function downloadCSV(data, filename) {
    ...
    // تجهيز العناوين
    var headers = ["التاريخ", "رقم السند", "نوع السند", "اسم الطرف", "نوع الطرف", __("Amount"), __("Account"), __("Payment Mode"), "الرقم المرجعي", __("Cost Center"), __("Status")];
